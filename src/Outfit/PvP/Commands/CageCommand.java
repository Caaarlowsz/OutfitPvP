   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
      if (!(sender instanceof Player)) {
         ...
         if (label.equalsIgnoreCase("cage") && player.hasPermission("zey.pvp.cage")) {
            if (args.length == 0) {
               player.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cErrado, utilize a sintaxe correta: /cage [Jogador(a)]");
               return true;
            }
            ...
            if (args.length == 1) {
               ...
               if (target != null) {
                  ...
                  player.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7Você prendeu o jogador(a): §e" + target.getName());
               } else {
                  player.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cEste jogador(a) está offline ou não existe.");
               }
            }
         }
