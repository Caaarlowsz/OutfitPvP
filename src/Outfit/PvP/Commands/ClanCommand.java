		...
		if(Cmd.getName().equalsIgnoreCase("clan")){
			...
			if(Args[0].equalsIgnoreCase("convidar")){
				...
				if(!list.contains(nick)){
					...
					if(vic !=null){
					vic.sendMessage(" ");
					vic.sendMessage("§5§lCLAN §8» §fVocê recebeu um Convite do clan §f"+ClanAPI.getClanPlayerString(p)+" §f("+ClanConfig.getConfig().getString(ClanAPI.getClanPlayerString(p)+".Tag").replace("&", "§")+"§f)");
					vic.sendMessage("§5§lCLAN §8» §fUse §d/clan aceitar "+ClanAPI.getClanPlayerString(p)+" §fou apenas ignore essa Mensagem");
					vic.sendMessage(" ");
					}
					ClanAPI.pedidos.put(ClanAPI.getClanPlayerString(p), list);
					ClanAPI.senMessage(ClanAPI.getClanPlayerString(p), "§d# §f"+nick+" §dfoi convidado para o Clan");
					return true;
				}else{
			...
			if(Args[0].equalsIgnoreCase("info")){
				...
				p.sendMessage("§b» §fTag do Clan: "+ClanConfig.getConfig().getString(ClanAPI.getClanPlayerString(p)+".Tag").replace("&", "§"));
			...
			if(Args[0].equalsIgnoreCase("expulsar")){
				...
				if(vic == null){
					...
					ClanAPI.senMessage(ClanAPI.getClanPlayerString(p), "§c# §f"+p.getName()+" §fexpulsou o Jogador §d"+vicoff.getName());
					return true;
				}
				...
				ClanAPI.senMessage(ClanAPI.getClanPlayerString(p), "§c# §f"+p.getName()+" §fexpulsou o Jogador §d"+vic.getName());
			...
			if(Args[0].equalsIgnoreCase("msg")){
		        ...
		        ClanAPI.senMessage(ClanAPI.getClanPlayerString(p), "§8[§d"+SPlayer.Clan(p)+"§8] §7"+p.getName()+" §8➸§f"+message.replace("&", "§"));
			}
			if(Args[0].equalsIgnoreCase("deletar")){
				...
				Bukkit.broadcastMessage("§5§lCLAN §8» §d"+p.getName()+" §fdeletou o Clan §8➸ §c"+ClanAPI.getClanPlayerString(p));
			...
			if(Args[0].equalsIgnoreCase("criar")){
				if(ZeyCoins.getPlayerMoney(p) < 2500){
					p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cÉ necessário ter 2500 de coins para criar um Clan");
					return true;
				}
				...
				String tag = Args[2].replace("&", "§").replace("§k", "").replace("§l", "").replace("§m", "").replace("§o", "").replace("§n", "");
		          String tag2 = ChatColor.stripColor(tag);
		          if ((tag2.length() > 3) || (tag2.length() < 3) || Args[2].length() > 9)
		          {
		            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cA tag do clan deve ter apenas 3 caracteres.");
		            return true;
		          }
