...
public class GmCommand implements CommandExecutor, Listener {
	public GmCommand(OutfitPvP main) {
	}

	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("gm")) {
			if (sender instanceof ConsoleCommandSender) {
				System.out.println(OutfitPvP.prefix + " §8➸ §cApenas jogadores podem usar isso.");
				return true;
			}

			Player p = (Player) sender;
			if (args.length == 0) {
				p.sendMessage(OutfitPvP.prefix + " §8➸ §cErrado, utilize a sintaxe correta: /gm [0/1]");
				...
				if (!p.hasPermission("zey.pvp.gm") && !p.isOp()) {
					...
					if (!args[0].equalsIgnoreCase("0") && !args[0].equalsIgnoreCase("sobrevivencia")) {
						p.sendMessage(OutfitPvP.prefix + " §8➸ §cGamemode Inexistente.");
					} else {
					...
					p.sendMessage(
							OutfitPvP.prefix + " §8➸ §7Seu gamemode foi alterado para §a§lCRIATIVO§7.");
				}
			}
		}
