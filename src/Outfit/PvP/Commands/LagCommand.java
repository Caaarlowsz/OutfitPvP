...
public boolean onCommand(CommandSender arg0, Command arg1, String arg2, String[] arg3) {
      ...
      if (arg1.getName().equalsIgnoreCase("ping")) {
         if (arg3.length == 0) {
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7Seu ping é de §d" + ((CraftPlayer)p).getHandle().ping + " §7ms");
            return true;
         } else {
            ...
            if (target == null) {
               ...
               p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7O jogador(a): §d" + target.getName() + " §7está com um ping de §d" + ((CraftPlayer)target).getHandle().ping + " §7ms");
               return true;
            }
         }
         ...
         p.sendMessage("§7Você está no Servidor§8: " + Main.prefix + "§7, online aqui§8: §d" + Bukkit.getOnlinePlayers().length);
         p.sendMessage("  §d- §7Uso de processamento§8: §d" + usedCPU + " %§8, §7lag§8: §d" + (usedCPU >= 90.0D ? "sim" : "nao"));
         p.sendMessage("  §d- §7Sobrecarregamento geral§8: §d" + RAM_USED_PERCENTAGE + " %§8, §7lag§8: §d" + (RAM_USED_PERCENTAGE >= 90.0D ? "sim" : "nao"));
         ...
         p.sendMessage(this.getString(RAM_USED_PERCENTAGE >= 90.0D ? "sim" : "nao", ping >= 450 ? "sim" : "nao", usedCPU >= 90.0D ? "sim" : "nao"));
         p.sendMessage("§7 ");
         return true;
      } else {
         return false;
   ...
   public static double getTPS(int ticks) {
      if (TickCount < ticks) {
         ...
         return (double)ticks / ((double)elapsed / 1000.0D) > 20.0D ? 20.0D : (double)ticks / ((double)elapsed / 1000.0D);
      }
   }
   ...
   public String getString(String lagram, String lagping, String lagprocess) {
      ...
      } else if (lagram.equalsIgnoreCase("sim") && lagping.equalsIgnoreCase("nao") && lagprocess.equalsIgnoreCase("nao")) {
         return "§2O Servidor e sua Internet estão em ótimo estado.";
      } else if (lagram.equalsIgnoreCase("sim") && lagping.equalsIgnoreCase("sim") && lagprocess.equalsIgnoreCase("nao")) {
         return "§6O Servidor e sua Internet estão em bom estado.";
      } else if (lagram.equalsIgnoreCase("sim") && lagping.equalsIgnoreCase("sim") && lagprocess.equalsIgnoreCase("sim")) {
         return "§cO Servidor e sua Internet estão em péssimo estado.";
      } else {
         return lagram.equalsIgnoreCase("nao") && lagping.equalsIgnoreCase("sim") && lagprocess.equalsIgnoreCase("nao") ? "§cSua Internet não está em boas condições" : "";
      }
   }
