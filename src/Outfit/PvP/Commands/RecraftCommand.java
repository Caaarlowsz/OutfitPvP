		...
		if (Cmd.getName().equalsIgnoreCase("recraft")) {
			if (WarpAPI.getWarp(p) == "Challenge") {
				p.sendMessage(OutfitPvP.prefix
						+ " §8➸ §cVocê não pode usar esse comando nestá Warp. Para utilizar esse comando, volte ao spawn e pegue um kit, usando o comando: /spawn");
				return true;
			}
			...
			p.sendMessage(OutfitPvP.prefix + " §8➸ §7Seu §a§lRECRAFT §7está a caminho.");
			Bukkit.getScheduler().scheduleSyncDelayedTask(Spawn.plugin, new Runnable() {
				public void run() {
					p.sendMessage(OutfitPvP.prefix + " §8➸ §aSeu recraft chegou.");
