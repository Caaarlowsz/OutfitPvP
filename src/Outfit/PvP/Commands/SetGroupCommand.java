...
public class SetGroupCommand implements CommandExecutor {
   public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
      ...
      if (cmd.getName().equalsIgnoreCase("setgroup")) {
         ...
         if (args.length != 2) {
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cErrado, utilize a sintaxe correta: /setgroup [Jogador(a)] [Grupo]");
            p.sendMessage("§cGrupos disponiveis§f: §7NORMAL§f, §bYT§f, §3YT+§f, §2BUILDER§f, §eHELPER§f, §dTRIAL§f, §1GC§f, §5MOD§f, §3MODGC§f, §5MOD+§f, §cADMIN§f, §cGERENTE§f, §3DEV§f, §fCOORDENADOR§f, §4DIRETOR§f, §4DONO§f.");
            return true;
         }
         ...
         if (grupo.equalsIgnoreCase("admingc")) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "pex user " + player.getName() + " group set AdminGC");
         ...
         if (sender.hasPermission("*") && grupo.equalsIgnoreCase("coordenador")) {
             ...
             Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "pex user " + player.getName() + " group set Coordenador");
         ...
         if (sender.hasPermission("*") && grupo.equalsIgnoreCase("diretor")) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "pex user " + player.getName() + " group set Diretor");
