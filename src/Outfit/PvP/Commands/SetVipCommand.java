...
public class SetVipCommand implements CommandExecutor {
   public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
      ...
      if (cmd.getName().equalsIgnoreCase("setvip")) {
         ...
         if (args.length == 0) {
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cErrado, utilize a sintaxe correta: /setvip [Jogador(a)] [Vip]");
            p.sendMessage("§cGrupos disponiveis§f: §7[NORMAL]§f, §a[VIP]§f, §9[MVP]§f, §6[PRO]§f, §e[SPRO]§f, §1[BETA]§f.");
            return true;
         }
         ...
         if (args.length == 2) {
            ...
            if (vipgroup.equalsIgnoreCase("spro")) {
               ...
               Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "pex user " + jogador.getName() + " group set spro");
