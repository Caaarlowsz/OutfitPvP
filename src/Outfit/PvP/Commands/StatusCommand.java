   ...
   public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
      ...
      if (cmd.getName().equalsIgnoreCase("status")) {
         if (args.length == 0) {
            ...
            p.sendMessage("§a");
            p.sendMessage("§5» §fMatou: §a" + Main.plugin.getConfig().getInt("status." + p.getName().toLowerCase() + ".kills"));
            p.sendMessage("§9» §fKillStreak: §6" + Main.plugin.getConfig().getInt("status." + p.getName().toLowerCase() + ".ks"));
            p.sendMessage("§3» §fMorreu: §c" + Main.plugin.getConfig().getInt("status." + p.getName().toLowerCase() + ".mortes"));
         ...
         if (args.length > 0 && args.length < 2) {
            ...
            if (t != null) {
               ...
               p.sendMessage("§a");
               p.sendMessage("§5» §fMatou: §a" + Main.plugin.getConfig().getInt("status." + t.getName().toLowerCase() + ".kills"));
               p.sendMessage("§9» §fKillStreak: §6" + Main.plugin.getConfig().getInt("status." + t.getName().toLowerCase() + ".ks"));
               p.sendMessage("§3» §fMorreu: §c" + Main.plugin.getConfig().getInt("status." + t.getName().toLowerCase() + ".mortes"));
