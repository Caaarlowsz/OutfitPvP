...
public class TpCommand implements CommandExecutor {
   ...
   public boolean onCommand(CommandSender Sender, Command Cmd, String Label, String[] Args) {
      ...
      if (Cmd.getName().equalsIgnoreCase("tp")) {
         ...
         if (Args.length == 0) {
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cErrado, utilize a sintaxe correta: /tp [Jogador(a)] [Alvo] ou (x) (y) (z)");
            return true;
         }
         ...
         if (Args.length > 2 || Args.length == 3) {
            ...
            if (!this.CheckarNumero(Args[1])) {
               p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cÉ necessário utilizar apenas números para teleportar para uma coordenada.");
               return true;
            }
            ...
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7Você foi §a§lTELEPORTADO §7até as coordenadas " + X + " " + Y + " " + Z);
            return true;
         }
         ...
         if (Args.length > 1) {
            ...
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cVocê teleportou o jogador(a): §e" + t.getName() + " §caté você!");
            return true;
         }

         p.teleport(t.getLocation());
         p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cVocê foi teleportado até o jogador(a): §e" + t.getName());
      }
