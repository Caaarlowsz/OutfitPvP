...
public class Gladiator implements Listener
{
        ...
        if (e.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.Gladiator.contains(p.getName())) {
                ...
                if (Gladiator.lutando.containsKey(p) || Gladiator.lutando.containsKey(r)) {
                    p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cVocê já está em uma arena Gladiator.");
                    return;
                }
                ...
                for (int bX = -10; bX <= 10; ++bX) {
                    for (int bZ = -10; bZ <= 10; ++bZ) {
                        for (int bY = -1; bY <= 10; ++bY) {
                            ...
                            if (!b.isEmpty()) {
                                p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cVocê não pode ultilizar esse kit aqui.");
                                return;
                            }
                            if (bY == 10) {
    ...
    void KitGladiatorSair(final PlayerQuitEvent e) {
        ...
        if (Gladiator.lutando.containsKey(p)) {
            ...
            q.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cO jogador(a): §e§l" + p.getDisplayName() + " §cdeslogou no gladiator.");
    ...
    void KitGladiatorMorrer(final PlayerDeathEvent e) {
        ...
        if (Gladiator.lutando.containsKey(p)) {
            ...
            m.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7Você §a§lGANHOU§7 a batalha no Gladiator.");
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §7Você §c§lPERDEU§7 a batalha no Gladiator.");
            Gladiator.lutando.remove(p);
    ...
    void KitGladiatorComando(final PlayerCommandPreprocessEvent e) {
        ...
        if (Gladiator.lutando.containsKey(p)) {
            e.setCancelled(true);
            p.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cVocê não pode executar comandos na Arena Gladiator.");
        }
    }
}
