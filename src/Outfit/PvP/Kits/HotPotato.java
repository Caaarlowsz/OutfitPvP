        ...
        if (e.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType().equals((Object)Material.POTATO) && Habilidade.getAbility(p).equalsIgnoreCase("HotPotato")) {
                if (Gladiator.lutando.containsKey(p.getName())) {
                    ...
                    k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §eVocê está com a tnt do hotpotato tire ou ira explodir em 5 segundos!");
                    k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §cClique com o botao direito na hotpotato para tira-la.");
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (HotPotato.emhotpotato.contains(k.getName())) {
                                k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §eVocê está com a tnt, ela será explodida em 4 segundos");
                            }
                        }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (HotPotato.emhotpotato.contains(k.getName())) {
                                k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §eVocê está com a tnt, ela será explodida em 3 segundos");
                            }
                        }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (HotPotato.emhotpotato.contains(k.getName())) {
                                k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §eVocê está com a tnt, ela será explodida em 2 segundos");
                            }
                        }
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (HotPotato.emhotpotato.contains(k.getName())) {
                                k.sendMessage(String.valueOf(Main.prefix) + " §8➸ §e§lVocê está com a tnt, ela será explodida em 1 segundo");
                            }
                        }
