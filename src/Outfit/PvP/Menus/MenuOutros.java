...
public class MenuOutros implements Listener, CommandExecutor
{
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equalsIgnoreCase("§8[§dOUTROS§8]") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
            
            if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase("§d§lCABEÇAS")) {
                e.setCancelled(true);
                p.closeInventory();
